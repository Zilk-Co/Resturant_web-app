...
import { sendDeliveryUpdate } from "@/lib/emailService";
...
export default function Orders() {
  ...
  const handleUpdateStatus = (id: string, status: string, phone?: string) => {
    updateStatus.mutate(
      { id, data: { status } },
      {
        onSuccess: () => {
          ...
          if (phone) {
            ...
            sendDeliveryUpdate(phone, id, statusLabels[status] || status).catch(() => {});
          }
        }
      }
    );
                  ...
                  <div className="p-6 lg:w-72 bg-muted/10 flex flex-col justify-between">
                    ...
                    <div className="space-y-3">
                      {order.status === 'Received' && (
                        <>
                          <Button size="lg" className="w-full font-bold bg-orange-600 hover:bg-orange-700 text-white" onClick={() => handleUpdateStatus(order.id, 'Preparing', order.customerPhone)} data-testid={`btn-prepare-${order.id}`}>
                            <ChefHat className="w-4 h-4 mr-2" /> Start Preparing
                          </Button>
                          <Button size="lg" variant="outline" className="w-full text-destructive border-destructive/50 hover:bg-destructive/10" onClick={() => handleUpdateStatus(order.id, 'Cancelled', order.customerPhone)}>
                            Cancel Order
                          </Button>
                        </>
                      )}
                      {order.status === 'Preparing' && (
                        <>
                          <Button size="lg" className="w-full font-bold bg-yellow-500 hover:bg-yellow-600 text-yellow-950" onClick={() => handleUpdateStatus(order.id, 'Ready', order.customerPhone)} data-testid={`btn-ready-${order.id}`}>
                      ...
                      {order.status === 'Ready' && (
                        <Button size="lg" className="w-full font-bold bg-primary hover:bg-primary/90 text-white" onClick={() => handleUpdateStatus(order.id, 'Delivered', order.customerPhone)} data-testid={`btn-deliver-${order.id}`}>
                      ...
                      <div className="pt-2 border-t mt-2">
                        ...
                        <Select value={order.status} onValueChange={(val) => handleUpdateStatus(order.id, val, order.customerPhone)}>
