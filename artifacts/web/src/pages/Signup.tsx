...
export default function Signup() {
  ...
  const recaptchaWidgetRef = useRef<any>(null);
  ...
  async function handleSignUp(e: React.FormEvent) {
    ...
    try {
      if (!auth) { setError("Firebase not loaded. Please refresh the page."); setLoading(false); return; }

      if (!recaptchaWidgetRef.current) {
        recaptchaWidgetRef.current = new (window as any).firebase.auth.RecaptchaVerifier("recaptcha-container", {
          size: "invisible",
        });
      }

      const result = await auth.signInWithPhoneNumber(fullPhone(), recaptchaWidgetRef.current);
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    if (!confirmationRef.current) { setError("Session expired. Please go back and try again."); return; }
    setLoading(true); setError("");
      ...
      <div className="w-full max-w-sm">
        ...
        {step === "form" ? (
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div>
              <label htmlFor="otp" className="block text-xs font-medium text-off-white-dim mb-1">6-Digit SMS Code</label>
            ...
            <div>
              <label htmlFor="sign-name" className="block text-xs font-medium text-off-white-dim mb-1">Full Name</label>
              <input id="sign-name" name="name" type="text" value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 text-jet text-sm focus:outline-none focus:border-crimson focus:ring-1 focus:ring-crimson/30 transition-all"
                placeholder="e.g. Muhammad Ali" />
            </div>
            <div>
              <label htmlFor="sign-user" className="block text-xs font-medium text-off-white-dim mb-1">Username</label>
              <input id="sign-user" name="username" type="text" value={username}
                onChange={(e) => setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g, ""))}
                className="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 text-jet text-sm focus:outline-none focus:border-crimson focus:ring-1 focus:ring-crimson/30 transition-all"
                placeholder="e.g. muhammad_ali" />
            </div>
            <div>
              <label htmlFor="sign-pass" className="block text-xs font-medium text-off-white-dim mb-1">Password</label>
              <div className="relative">
                <input id="sign-pass" name="password" type={showPassword ? "text" : "password"} value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 text-jet text-sm focus:outline-none focus:border-crimson focus:ring-1 focus:ring-crimson/30 transition-all pr-10"
                  placeholder="At least 6 characters" />
                <button type="button" onClick={() => setShowPassword(!showPassword)}
