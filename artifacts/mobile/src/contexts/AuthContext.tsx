...
async function refreshAccessToken(): Promise<string | null> {
  try {
    const res = await fetch(`${customFetch.getBaseUrl()}/api/auth/refresh`, {
      method: "POST",
    ...
    if (accessToken) {
      await AsyncStorage.setItem("thb_access_token", accessToken);
    }
    return accessToken;
  } catch {
    return null;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const storedToken = await AsyncStorage.getItem("thb_access_token");
        if (storedToken) {
          accessToken = storedToken;
          const u = await fetchWithAuth<User>(
            `${customFetch.getBaseUrl()}/api/auth/me`
          );
          setUser({ ...u, loyaltyPoints: u.loyaltyPoints ?? 0 });
        }
      } catch {
        accessToken = null;
        await AsyncStorage.removeItem("thb_access_token");
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const requestOtp = useCallback(async (phone: string) => {
    const baseUrl = customFetch.getBaseUrl();
  ...
  const verifyOtp = useCallback(async (phone: string, code: string, name?: string) => {
    ...
    const data = await res.json();
    accessToken = data.accessToken;
    await AsyncStorage.setItem("thb_access_token", data.accessToken);
  ...
  const signIn = useCallback(async (name: string, phone: string) => {
    ...
    await AsyncStorage.setItem("thb_access_token", data.token);
    setUser({
      id: data.id,
      name: data.name,
      phone: data.phone,
  ...
  const signOut = useCallback(async () => {
    try {
      const baseUrl = customFetch.getBaseUrl();
      await fetchWithAuth(`${baseUrl}/api/auth/logout`, { method: "POST" });
    } catch {}
    accessToken = null;
    await AsyncStorage.removeItem("thb_access_token");
    setUser(null);
  ...
  const updateProfile = useCallback(
    async (updates: Partial<Pick<User, "name" | "phone" | "email" | "profilePicUrl" | "loyaltyPoints">>) => {
      const baseUrl = customFetch.getBaseUrl();
      const u = await fetchWithAuth<User>(`${baseUrl}/api/auth/profile`, {
        method: "PUT",
  ...
  const addAddress = useCallback(async (address: Omit<SavedAddress, "id">) => {
    const baseUrl = customFetch.getBaseUrl();
    const addr = await fetchWithAuth<SavedAddress>(`${baseUrl}/api/auth/addresses`, {
      method: "POST",
  ...
  const updateAddress = useCallback(
    async (id: string, updates: Partial<Omit<SavedAddress, "id">>) => {
      const baseUrl = customFetch.getBaseUrl();
      const addr = await fetchWithAuth<SavedAddress>(`${baseUrl}/api/auth/addresses/${id}`, {
        method: "PUT",
        body: JSON.stringify(updates),
      });
      setUser((prev) => {
  ...
  const removeAddress = useCallback(async (id: string) => {
    const baseUrl = customFetch.getBaseUrl();
    await fetchWithAuth<void>(`${baseUrl}/api/auth/addresses/${id}`, { method: "DELETE" });
    setUser((prev) => {
      if (!prev) return prev;
  ...
  return (
    <AuthContext.Provider
      value={{
        user, isLoading, requestOtp, verifyOtp, signIn, signOut, updateProfile,
